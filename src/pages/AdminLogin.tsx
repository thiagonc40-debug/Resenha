...
export function AdminLogin() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // useStore auth listener will handle navigation if user is admin
      // but just in case, wait a bit
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // useStore auth listener handles navigation
      setTimeout(() => navigate('/admin/dashboard'), 1000);
    } catch (err: any) {
      setError(err.message || 'Falha ao autenticar com E-mail/Senha.');
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-surface flex items-center justify-center p-space-md">
      <div className="w-full max-w-md bg-surface-container-low rounded-2xl p-space-xl shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col gap-space-md relative z-10">
          <p className="text-body-sm text-on-surface-variant">Entre com suas credenciais de administrador.</p>
          ...
          <form onSubmit={handleEmailLogin} className="flex flex-col gap-3">
            <div>
              ...
              <input 
                ...
                className="w-full bg-surface-container px-3 py-2 rounded-lg border border-surface-container-highest focus:border-primary focus:outline-none"
                placeholder="admin@email.com"
                required
              />
            </div>
            <div>
              <label className="block text-body-sm text-on-surface-variant mb-1">Senha</label>
              <input 
                type="password" 
                value={password}
            ...
            <button 
              type="submit"
              disabled={isLoading}
              className="w-full py-3 rounded-xl bg-primary hover:bg-primary-container text-on-primary font-bold text-headline-sm transition-transform shadow-md flex items-center justify-center gap-2 mt-2"
            >
          ...
          <div className="flex items-center gap-3 my-2">
            ...
            <span className="text-body-sm text-on-surface-variant font-bold uppercase">Ou</span>
            <div className="h-[1px] flex-1 bg-surface-container-highest"></div>
          </div>
